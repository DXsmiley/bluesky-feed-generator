...
def decay_curve(x: float) -> float:
    ALPHA = 1.5
    return 1 / (x**ALPHA) if x > 1 else (2 - (1 / ((2 - x) ** ALPHA)))


async def get_like_counts(
    db: Database, run_starttime: datetime, filter: prisma.types.LikeWhereInput
) -> Dict[str, int]:
    likes_aggregated = await db.like.group_by(
        ...
        where={
            "created_at": {
                "lt": run_starttime,
                "gt": run_starttime - LOOKBACK_HARD_LIMIT,
            },
            "post": {
                "is": {
                    "indexed_at": {
    ...
    return {
        i.get("post_uri", ""): i.get("_count", {}).get("_all", 0)
        for i in likes_aggregated
    }
...
async def load_all_posts(db: Database, run_starttime: datetime) -> List[PostWithInfo]:
    likes_by_furries = await get_like_counts(
        db, run_starttime, {"liker": {"is": server.database.user_is_in_fox_feed}}
    )
    likes_by_girls = await get_like_counts(
        db, run_starttime, {"liker": {"is": server.database.user_is_in_vix_feed}}
    )
...
def penalty(p: PostWithInfo):
    ...
    return (
        ...
        (
            0.7
            if p.post.media_count > 0 and p.post.media_with_alt_text_count == 0
            else 1.0
        )
...
def raw_score(run_starttime: datetime, p: PostWithInfo) -> float:
    return _raw_score(
        run_starttime - p.post.indexed_at, p.like_count_furries
    ) * penalty(p)
...
def raw_vix_vote_score(run_starttime: datetime, p: PostWithInfo) -> float:
    return (
        0
        if p.like_count_girls < 3
        else _raw_score(run_starttime - p.post.indexed_at, p.like_count_girls)
        * penalty(p)
    )
...
class FeedParameters:
    ...
    remix_func: Callable[
        [List[Tuple[float, PostWithInfo]]], List[PostWithInfo]
    ] = lambda p: [i for _, i in p]
...
def sorted_by_score_desc(
    posts: List[Tuple[float, PostWithInfo]]
) -> List[Tuple[float, PostWithInfo]]:
...
def top_100_chronological(p: List[Tuple[float, PostWithInfo]]) -> List[PostWithInfo]:
    return sorted(
        [i for _, i in p][:100], key=lambda p: p.post.indexed_at, reverse=True
    )
...
async def create_feed(db: Database, fp: FeedParameters, rd: RunDetails) -> None:
    ...
    scored_posts = sorted_by_score_desc(
        [
            ...
            for index, (post_raw_score, post) in enumerate(
                sorted_by_score_desc(just_by_author)
            )
            if post_raw_score > 0
        ]
    )
    ...
    for i, post in enumerate(will_store):
        ...
        try:
            ...
            cprint(
                f"Unique PostScore violation error on {post.post.uri}::{fp.feed_name}::{rd.run_version} ({uri_count} instances of this URI)",
                "red",
                force_color=True,
            )
...
def post_is_irl_nsfw(p: PostWithInfo) -> bool:
    ...
    return (
        ...
        or "murrsuit" in t
        or "porn" in t
    ) and not ("art" in t or "commission" in t or "drawing" in t)
...
async def score_posts(db: Database) -> None:
    ...
    cprint(
        f"Scoring round {run_version} has {len(all_posts)} posts and {total_likes} likes and {total_girl_likes} girl-likes",
        "yellow",
        force_color=True,
    )
